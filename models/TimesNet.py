...
class TimesBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            # padding
            if (self.window_len + self.pred_len) % period != 0:
                length = (
                    ((self.window_len + self.pred_len) // period) + 1) * period
                padding = torch.zeros([x.shape[0], (length - (self.window_len + self.pred_len)), x.shape[2]]).to(x.device)
                out = torch.cat([x, padding], dim=1)
            else:
                length = (self.window_len + self.pred_len)
                out = x
            # reshape
            ...
            res.append(out[:, :(self.window_len + self.pred_len), :])
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.predict_linear = nn.Linear(
            self.window_len, self.pred_len + self.window_len)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        DataEmbedding 
        x_mark는 window_len 내에서  label_len 만큼의 예측에 영향을 줄 어느 정도의 길이.
